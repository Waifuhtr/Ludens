...
object SettingsPresetFactory {
    ...
    /** Parses supported action tokens (`settings`, `controls`, `mute`, `fps`, `webgl`, `cheats`, `plugins`). */
        ...
        return raw.split(',', ';', '|')
            ...
            .mapNotNull { token ->
                when (token) {
                    ...
                    "cheats", "cheat", "cheatmenu" -> ActionType.Cheats
                    "plugins", "plugin", "pluginmanager" -> ActionType.Plugins
                    else -> null
                }
            }
            .toSet()
