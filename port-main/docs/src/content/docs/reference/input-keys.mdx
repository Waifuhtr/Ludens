...
:::note
On Android, the system back button/gesture is forwarded to the game as `Escape` (27) while on
the Home screen, rather than immediately exiting the app — matching how `Escape` behaves in a
desktop build. Pressing back again shortly after (with nothing left to cancel) shows the exit
confirmation instead. This does not require any `Input Key` configuration; it is handled
independently of the on-screen control buttons.
:::
