...
$("btnUpload").addEventListener("click", async () => {
  ...
  try {
    const st = await api("/project/upload", { method: "POST", headers: headers(), body });
    projectId = st.id;
    log(`Analiz tamam — ${st.engine.toUpperCase()}, ${st.total_units} benzersiz metin, ${st.total_slots} konum.`);
...
const STATUS_TR = {
  ...
  cancelled: "durduruldu", failed: "hata", extracting: "çıkarılıyor",
};
...
function render(st) {
  ...
  $("projectStats").innerHTML = [
    stat(st.engine ? st.engine.toUpperCase() : "-", "Motor"),
...
async function refresh() {
  if (!projectId) return;
  try {
    const st = await api(`/project/${projectId}/status`, { headers: headers() });
...
$("btnStart").addEventListener("click", async () => {
  ...
  try {
    ...
    await api(`/project/${projectId}/start`, {
      method: "POST",
...
$("btnCancel").addEventListener("click", async () => {
  try {
    await api(`/project/${projectId}/cancel`, { method: "POST", headers: headers() });
...
$("btnDownload").addEventListener("click", async () => {
  ...
  try {
    ...
    const res = await fetch(`${apiBase()}/project/${projectId}/download`, { headers: headers() });
    ...
    log(`İndirildi (${(blob.size / 1048576).toFixed(2)} MB).`);
  } catch (e) {
...
$("btnDelete").addEventListener("click", async () => {
  if (!projectId) return;
  try {
    await api(`/project/${projectId}`, { method: "DELETE", headers: headers() });
    log("Proje silindi.");
    stopPolling();
    projectId = null;
...
(async () => {
  try {
    const list = await api("/project", { headers: headers() });
    if (list.projects && list.projects.length) {
      ...
      log("Arayüz hazır. Bir data klasörü zip'i yükleyin.");
    }
  } catch (e) {
